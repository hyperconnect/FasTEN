...
parser.add_argument('--use_oracle', default=False, type=bool, help='Use oracle corruption matrix or not.')
...
def test(epoch, net, loader, mode='test'):
    ...
    if mode == 'valid':
        ...
        tb_writer.add_summary(best_prec1_test, f"test/Test_acc@best_valid", epoch)
...
if __name__ == '__main__':
    ...
    tb_path = Path(os.path.join(save_dir, "tb_logs", args.method, args.exp))
    if not os.path.exists(tb_path):
        os.makedirs(tb_path)
    tb_writer = TensorboardWriter(tb_path)

    stats_log = open(f'{save_dir}/%s_%.1f_%s' % (args.dataset, args.r, args.noise_mode) + '_stats.txt', 'w')
